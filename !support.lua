...
script_changelog = [[	v1.07 [21.08.2018]
* В связи с фризами у нек. пользователей была вырезана криптографическая проверка лицензии. Проверка лицензии теперь отвязана от сервера. Чтобы купить лицензию, теперь необходимо связаться с автором.
* Если у вас наблюдается вылет getCurrentHotKey, попробуйте отключить хоткеи, которыми не пользуется. Автор библиотеки удалил тему, а переписывать его творение нет времени.
...
function chklsn()
  ...
  if sampGetPlayerNickname(myid) == decode("368933181e42c0a15e0c55dd41833d57ee5a") then
    licensenick = decode("368933181e42c0a15e0c55dd41833d57ee5a")
    mode = decode("b7adc636d2d621")
    licenseserver = decode("2ddece585a3a3d317505dc8020c1")
    PROVERKA = true
  end
	if sampGetPlayerNickname(myid) == decode("7d78c5bef30d56f48a16cbfcf41f50") then
		licensenick = decode("7d78c5bef30d56f48a16cbfcf41f50")
		mode = decode("b7adc636d2d621")
		licenseserver = decode("2ddece585a3a3d317505dc8020c1")
		PROVERKA = true
	end
	if sampGetPlayerNickname(myid) == decode("3cf97cbe2018736ff612ef") then
    licensenick = decode("3cf97cbe2018736ff612ef")
    mode = decode("b7adc636d2d621")
    licenseserver = decode("2ddece585a3a3d317505dc806b32")
    PROVERKA = true
  end
  if sampGetPlayerNickname(myid) == decode("caadc611d2453dfa1908") then
    licensenick = decode("caadc611d2453dfa1908")
    mode = decode("b7adc636d2d621")
    licenseserver = decode("2ddece585a3a3d317505dc806b32")
    PROVERKA = true
  end
  if PROVERKA ~= true then
    sampAddChatMessage("Проверка не пройдена. Обратитесь в поддержку: vk.me/qrlk_mods", - 1)
    print(3 > afsdfs)
  end
end
